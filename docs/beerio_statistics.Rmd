...
extensions = 'Responsive',
  options = list(
    ...
    autoWidth = TRUE,
    fillContainer = TRUE,
    pageLength = 10, paging=TRUE, info=TRUE, searching=FALSE, lengthChange=FALSE,
    aoColumnDefs = list(
      list(className = 'dt-center',  targets = c(1), orderData = 7),
      list(bSortable = TRUE, sType = 'NumericOrBlank', className = 'dt-center', targets = c(2,3,4,5,6)),
...
beverages <-
  ...
  arrange(Date) %>%
  mutate(Date = str_replace(format(Date, "%A, %B %e, %Y"), "  ", " ")) %>%
  pivot_longer(-Date, names_to = "Name", values_to = "Beverage") %>%
...
beerio_table %>%
  select(Name, `Games<br>Played`, Victories, `Win<br>Percentage`, `Times<br>Punished`, `Times<br>Punisher`) %>%
  datatable(options = list(pageLength = 10, searching=FALSE, lengthChange=FALSE, paging=FALSE, info=FALSE,
                           columnDefs = 
                             list(
...
extensions = 'Responsive',
  options = list(
    ...
    pageLength = 10, paging=TRUE, info=TRUE, searching=FALSE, lengthChange=FALSE,
    aoColumnDefs = list(
      list(className = 'dt-center',  targets = c(0), orderData = 9),
      list(width = 40, bSortable = TRUE, sType = 'NumericOrBlank', className = 'dt-center', targets = c(1,2,4,5,6,7)),
      list(width = 50, bSortable = TRUE, sType = 'NumericOrBlank', className = 'dt-center', targets = c(3)),
      list(width = 90, bSortable = TRUE, sType = 'NumericOrBlank', className = 'dt-center', targets = c(0)),
      list(targets = 9, visible =FALSE))
    ), 
  escape=FALSE, autoHideNavigation=TRUE, rownames=FALSE,
  style = 'bootstrap', class = "compact") %>%
  DT::formatStyle(columns = 1:9, fontSize = '80%')
...
map_data_table <- map_data %>% ungroup() %>% 
  ...
  pivot_wider(names_from = Player, values_from = `Average Score`) %>% mutate(Map = str_replace(Map, "\n", " "))
...
DT::datatable({
  ...
  options = list(
    ...
    pageLength = 10, paging=TRUE, info=TRUE, searching=FALSE, lengthChange=FALSE,
    aoColumnDefs = list(
      list(bSortable = TRUE, sType = 'NumericOrBlank', className = 'dt-center', targets = 0:4)
    )), 
  escape=FALSE, autoHideNavigation=TRUE, rownames=FALSE,
  style = 'bootstrap', class = "compact") %>%
  DT::formatStyle(columns = c(1, 2, 3, 4, 5), fontSize = '100%')
```
