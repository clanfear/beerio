...
options = list(
  ...
  aoColumnDefs = list(
    list(width = "45px",  targets = c(2)),
    list(width = '45px',  bSortable = TRUE, sType = 'NumericOrBlank', targets = c(2,3,4,5)),
    list(width = '220px', bSortable = TRUE, sType = 'NumericOrBlank', targets = c(6)))
...
The next plot depicts overall score distributions for each player. Wider cross-sections indicate more games with scores in that range. Points indicate scores for each match, jittered horizontally to reduce overplotting.
...
The last plot depicts the overall score distribution with mean (`r round(mean_score,1)`) and median (`r round(median_score,1)`).
