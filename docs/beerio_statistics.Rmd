...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10, fig.align='center', dev="png", dpi = 200}
beerio_data %>%
  group_by(Date) %>%
  filter(max(Game) > 1) %>%
  ungroup() %>%
...
The first plot show average scores for each player on each map. Courses are sorted by their overall mean score--the map with the current highest mean score is `r str_replace(levels(map_data$Map)[8], "\n", " ")`.
...
```{r, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10, fig.align='center', dev="png", dpi = 200}
# ggplot(map_data, aes(x = `Average Score`, y = Map, group = Player, fill = Player)) +
#   scale_fill_manual("",values = c("Chuck" = "#F8766D", "Neil" = "#7CAE00", "Shawn" = "#00BFC4", "Steve"="#C77CFF")) +
#   geom_col(position="dodge") + theme_mk8() + theme(legend.position = "bottom") + ylab("") + ggtitle("Average Scores by Map", subtitle = "For each player") +
#   scale_x_continuous(breaks = scales::pretty_breaks(3))
```
