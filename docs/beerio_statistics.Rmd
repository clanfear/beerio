...
beerio_table <- beerio_data %>% 
  ...
  summarize(`Games<br>Played` = n(), 
            `Overall<br>Average` = round(mean(Score),1), 
...
ggplot(beerio_data, aes(x = Game, y = Score)) + 
  geom_point(data = beerio_data %>%  
               filter(is.na(Punished)), aes(fill=Name), size =3, shape  = 21, color = "black", position = position_jitter(height=0, width=0.1)) + 
  ...
  scale_color_discrete("") +
    geom_image(data = beerio_data %>%  
             ...
             aes(x = Game, y = Score, image = Image), 
             size = .05, inherit.aes = FALSE, position = position_jitter(height=0, width=0.1)) +
...
beerio_data %>%
  ...
  geom_bar(data = beerio_data %>% filter(Give_and_Take=="Take"), fill = "#7CAE00", width =1) +
  theme_mk8() +
  ylab("") +
