...
beerio_table <- beerio_data %>% 
  ...
  mutate(`Punishment Ratio` = signif(`Times<br>Punisher`/`Times<br>Punished`,3)) %>%
  left_join(winners %>% 
              ungroup() %>% 
              filter(Winner != "Tie") %>% 
              count(Name = Winner) %>% 
              ...
              rename(Victories = n)) %>%
    mutate(`Wins per Punishment` = signif(Victories / `Times<br>Punished`,3)) %>%
...
beerio_table %>%
  select(Name, `Games<br>Played`, Victories, `Win<br>Percentage`, `Times<br>Punished`, `Times<br>Punisher`,`Punishment Ratio`, `Wins per Punishment`) %>%
  beerio_datatable()
```
...
map_max <- beerio_rounds %>% pivot_longer(Neil:Karly) %>%
  ...
  group_by(Map) %>%
  summarize(`Best Score` = score, `Best Player(s) (Times)` = paste(name_times, collapse = " & "))
